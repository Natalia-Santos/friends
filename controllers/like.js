...
function prueba(req, res){ 
	return res.status(200).send({message: 'Hola desde elo control de likes'});
} 
...
/*function updateLike(req, res){
	...
	// res.status(200).send({update:update});
		 	...
		 	Publication.findByIdAndUpdate(publicationId, update, {new:true}, (err, LikeUpdated) => {
				if(err) return res.status(500).send({message: 'Error en la petición'});
...
function userLike(req, res){
    ...
    query.exec(function (err, UserFind) {
        ...
        return res.status(200).send({encontrado: UserFind});
		 ...
		 /*	Like.find({user: userId}, (err, UserFind) => {
				if(err) return res.status(500).send({message: 'Error en la petición'});
